...
if (isset($_POST['submit'])) {
  ...
  if ($id == 0) {
    $id0 = "<p>Please Select Employee First.</p><br>";
    exit();
  }
  if ($presence == 1) {
    $totaltime = sprintf('%02d:%02d:%02d', $hours, $mins, $secs);
    if ($timein == 0 or $timein == " ") {
      $nulltimein = "<p>Enter Entry Time.</p><br>";
      exit();
    }
    if ($timeout == 0 or $timeout == "") {
      $nulltimeout = "<p>Enter Exit Time.</p><br>";
      exit();
    }
...
<?php include 'nav.php'?>
  ...
  <div class="dailyupdates">
    ...
    <form method="post" class="updateform">
      ...
      <input type="number" name="expenses"> <br> <input type="submit" name="submit"> <label class="check" name="massage">
      <?php
      if($fn == true){
